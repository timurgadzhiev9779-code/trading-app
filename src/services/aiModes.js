// AI Trading Modes Configuration
...
export const AI_MODES = {
    CONSERVATIVE: {
      ...
      settings: {
        ...
        maxPositions: 2,             // Максимум 2 позиции
        riskPerTrade: 0.005,         // 0.5% риска на сделку
    ...
    BALANCED: {
      ...
      settings: {
        ...
        regimes: ['BULL_MARKET', 'ACCUMULATION', 'BREAKOUT'],
    ...
    AGGRESSIVE: {
      ...
      settings: {
        ...
        strategies: ['TREND_FOLLOWING', 'MOMENTUM', 'MEAN_REVERSION', 'BREAKOUT'],
        
        regimes: ['BULL_MARKET', 'ACCUMULATION', 'BREAKOUT', 'HIGH_VOLATILITY', 'RANGING'],
        
        stopLossATR: 2.5,
        takeProfitATR: 5.0,
        trailingStop: true,
        partialTP: [0.3, 0.3, 0.4]
      }
    }
  }
  ...
  export function validateSignal(signal, mode) {
    ...
    if (signal.confidence < settings.minConfidence) {
      return { 
        valid: false, 
        reason: `Уверенность ${signal.confidence}% < ${settings.minConfidence}%` 
      }
    }
    
    // Проверяем режим рынка
    if (signal.regime && !settings.regimes.includes(signal.regime)) {
      return { 
        valid: false, 
        reason: `Режим рынка ${signal.regime} не разрешён` 
      }
    }
    ...
    if (required.ml && signal.mlConfidence < required.ml) {
      return { 
        valid: false, 
        reason: `ML ${signal.mlConfidence}% < ${required.ml}%` 
      }
    }
    ...
    if (required.patterns && signal.patternsCount < required.patterns) {
      return { 
        valid: false, 
        reason: `Паттернов ${signal.patternsCount} < ${required.patterns}` 
      }
    }
    ...
    if (required.orderBook && signal.buyPressure < required.orderBook) {
      return { 
        valid: false, 
        reason: `Order Book ${signal.buyPressure}% < ${required.orderBook}%` 
      }
    }
    ...
    if (required.whales && signal.whaleActivity !== required.whales) {
      return { 
        valid: false, 
        reason: `Киты: ${signal.whaleActivity} != ${required.whales}` 
      }
    }
  ...
  export function selectStrategy(analysis, mode) {
    ...
    return {
      ...
      tpMultiplier: settings.takeProfitATR / settings.stopLossATR,
      slMultiplier: 1.0,
      confidenceBonus: 0
    }
  }
  ...
  export function calculatePositionSize(portfolio, analysis, mode, recentPerformance) {
    const settings = mode.settings
    let baseSize = portfolio.available * settings.positionSize
    let multiplier = 1.0
